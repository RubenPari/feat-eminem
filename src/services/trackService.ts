...
export async function searchTracksEminem(): Promise<SpotifyApi.TrackObjectFull[]> {
    const limit = 50;
    let offset = 0;
    ...
    while (offset < maxOffset) {
        const searchTracksEminem = await client.searchTracks('Eminem', {limit, offset});
...
export async function searchTracksFeatEminem(): Promise<SpotifyApi.TrackObjectFull[]> {
    const limit = 50;
    let offset = 0;
    const maxOffset = 1000;
    ...
    while (offset < maxOffset) {
        const searchTracksFeatEminem = await client.searchTracks('feat. Eminem', {limit, offset});
...
export async function searchTracksWithEminem(): Promise<SpotifyApi.TrackObjectFull[]> {
    const limit = 50;
    let offset = 0;
    const maxOffset = 1000;
    ...
    while (offset < maxOffset) {
        const searchTracksWithEminem = await client.searchTracks('with Eminem', {limit, offset});
...
export async function getAllTracksD12(): Promise<SpotifyApi.TrackObjectSimplified[]> {
    ...
    for (const album of albums) {
        let albumTracks = await client.getAlbumTracks(album.id).then((res) => res.body.items);
...
export async function getAllTracksBadMeetsEvil(): Promise<SpotifyApi.TrackObjectSimplified[]> {
    const badMeetsEvilArtist = await client.searchArtists('Bad Meets Evil').then((res) => {
        return res.body.artists!.items[0];
    });

    // get all albums from Bad Meets Evil
    const albums = await client.getArtistAlbums(
        badMeetsEvilArtist.id,
        {include_groups: 'include_groups=album,single'})
        .then((res) => res.body.items);
...
export async function removeDuplicateTracks(tracks: {
    ...
    let filteredTracks = tracks.filter((track) => {
        return !track.name.includes(INTERNATIONAL_RADIO_EDIT) &&
    ...
    filteredTracks = filteredTracks.filter((track) => {
        return !filteredTracks.find((t) => {
            return t.name.includes('Remaster') && t.name.includes(track.name) && t.name !== track.name;
        });
    });
